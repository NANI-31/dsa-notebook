...
const Algorithms: React.FC = () => {
  ...
  const activeAlgo = searchParams.get("view");
  
  const { taxonomyStats, status } = useSelector((state: RootState) => state.categories);

  const algorithmStats = useMemo(() => {
    const algosCategory = taxonomyStats.find(c => c.name === "Algorithms");
    return algosCategory ? algosCategory.subCategories : [];
  }, [taxonomyStats]);
  ...
  useEffect(() => {
    if (algorithmStats.length > 0) {
      const topCategoryNames = algorithmStats.slice(0, 6).map(c => c.name);
    ...
    return (
      <Suspense fallback={
        <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
           ...
           <p className="text-text-muted font-medium">Priming algorithm library...</p>
        </div>
      }>
        <ProblemExplorer 
          categoryName="Algorithms" 
          subCategory={activeAlgo} 
          onBack={handleBack} 
        />
      </Suspense>
    );
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-text-muted font-medium max-w-2xl text-balance">
            Strategic patterns for problem-solving. Master the logic that drives efficient computation and optimization.
          </p>
        </div>
      ...
      {algorithmStats.length === 0 && status !== "loading" && (
        <div className="bg-sidebar border border-dashed border-border-subtle p-12 rounded-4xl flex flex-col items-center text-center space-y-4 shadow-inner">
          ...
          <div className="space-y-1">
            <h3 className="text-xl font-bold text-text-main">No Algorithms Found</h3>
            <p className="text-text-muted max-w-sm">
              We couldn't find any algorithm categories. Try resetting your database or adding them manually in Taxonomy.
            </p>
          </div>
        </div>
      )}
